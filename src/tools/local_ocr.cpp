...
OcrResult recognizeLocal(const QImage& source) {
    ...
    try {
        ...
        if(!engine) return {{},QCoreApplication::translate("MouseWheel","未安装可用的 Windows OCR 语言，请在系统语言设置中安装对应语言的文字识别组件。")};
        ...
        if(image.isNull()) return {{},QCoreApplication::translate("MouseWheel","识别图片为空。")};
        ...
        return {{},QCoreApplication::translate("MouseWheel","本地 OCR 失败：%1").arg(QString::fromWCharArray(error.message().c_str()))};
    }
}
}
